...
A simple cell parsing function reads trough the cell and return every
line with a ‘=’ in it.
...
The simple parsing function loses all information about the datatypes,
since it returns the read rows as strings. On top of that, determining
the expressions by ‘=’ is convenient but is wrong with conditional
statements and struggles with a lot of python syntax.
...
The parsed cell content gets inserted in the sympy.sympify function
after some string manipulation. The sympified string is inserted in a
markdown math environment. The put_out function is capable of the
following:
...
The symbolic representation uses Sympy.sympify, which afterwards creates
a latex string out of the equation.
